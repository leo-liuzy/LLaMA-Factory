...
def parse_args():
    ...
    parser.add_argument(
        "--test-set-choice", type=str, default="test_id", choices=["test_id", "test_ood-both"],
    ...
    parser.add_argument(
        "--question_key", type=str, default="text",
        help="Question key"
    )
    parser.add_argument(
        "--first_n_tokens", type=int, default=5,
        help="Question key"
    )
...
def load_controlled_RE_data(args, file_path):
    ...
    lst = []
    for s in samples:
        text = s[args.question_key]
        q = tokenizer.decode(tokenizer.encode(text)[:args.first_n_tokens])
        a = tokenizer.decode(tokenizer.encode(text)[args.first_n_tokens:args.max_tokens+args.first_n_tokens])
        lst.append({"question": q, "answer": a})
...
def clean_value(value):
    if isinstance(value, str):
        # Remove control chars (below ASCII 32 except tab/newline)
...
def main():
    ...
    for eval_data_name in eval_data_names:
        ...
        output_file = f"{save_dir}/{eval_data_name}_{args.test_set_choice}_debug_{args.first_n_tokens}_{args.question_key}_{args.context_type}_temp{args.temperature}.xlsx"
